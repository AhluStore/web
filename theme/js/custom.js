...
// }
	...
	function countDown(){
	    if ($("#countdown1").length > 0) {
	        $('#countdown1').countdown((new Date(2019, 12, 1)).getTime(), {elapse: true})
	...
	$(document).ready(function() {
		 //countdown
            $(".countdown").each(function(i,v){
               var liftoffTime = new Date($(v).data("time")); 


               if(liftoffTime.toString()!=='Invalid Date'){
               	 v.format = $(v).html();
               	 $(v).html();
                  $(v).countdown(liftoffTime.getTime(), {elapse: true})
					.on('update.countdown', function(event) {
					  var $this = $(this);
					  if (event.elapsed) {
					    $this.html(event.strftime(this.format));
					  } else {
					    $this.html(event.strftime(this.format));
					  }
					});
	...
	function productCarousel(carousel,numberXl,numberLg,numberMd,numberSm,numberXs) {
		...
		if (carousel.closest(".ahlu-widget").find('.carousel-products__button').length > 0) {
	
				carousel.slick({
						prevArrow: carousel.closest(".ahlu-widget").find('.carousel-products__button .btn-prev'),
						nextArrow: carousel.closest(".ahlu-widget").find('.carousel-products__button .btn-next'),
						dots: true,
	...
	function elevateZoom() {
		...
		$('.product-main-image').on('click','.product-main-image__item', function(){
